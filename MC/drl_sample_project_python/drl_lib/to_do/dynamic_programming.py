from MC.drl_sample_project_python.drl_lib.do_not_touch.contracts import MDPEnv
...
def policy_evaluation_on_line_world(pi: np.ndarray) -> ValueFunction:
    ...
    """
    S = LineWorld(MDPEnv).states()
    A = LineWorld(MDPEnv).actions()
    R = LineWorld(MDPEnv).rewards()
    theta = 0.0000001
            ...
            for a in A:
                ...
                for s_p in S:
                    for r in range(len(R)):
                        total += LineWorld(MDPEnv).transition_probability(s, a, s_p, r) * (R[r] + 0.999 * V[s_p])
                total *= pi[s, a]
                V[s] += total
